...
# -----------------------
st.set_page_config(page_title="PMSB - Esgoto Sanitário", layout="wide")
st.title("💧 PMSB de Augusto Corrêa - PA: Esgoto Sanitário")

# Caminho do GeoJSON (na mesma pasta do script)
DATA_PATH = Path(__file__).parent / "BD_BAIRROS_E_ZONA_RURAL_CONSUMO_ALL_ESGOTO.geojson"
...
def load_data(geojson_path: Path):
    if not geojson_path.exists():
        raise FileNotFoundError(
            ...
            "Coloque o BD_BAIRROS_E_ZONA_RURAL_CONSUMO_ALL_ESGOTO.geojson na raiz do app."
        )
    ...
    text_cols = ["AREA_y", "BAIRRO_COM", "LABEL_13", "LABEL_Q14", "LABEL_Q15", "LABEL_Q16", "LABEL_Q17", "LABEL_Q18"]
    for c in text_cols:
        if c not in df.columns:
            df[c] = "Não informado"
        df[c] = df[c].fillna("Não informado").astype(str)

    num_cols = ["N_domi", "pop_estim"]
    for c in num_cols:
        if c not in df.columns:
            df[c] = 0
...
with col2:
    st.metric("População Estimada", f"{df['pop_estim'].sum():,.0f}")


# -----------------------
# GRÁFICOS
# -----------------------
st.markdown("### 📈 Indicadores de Esgoto Sanitário")
col1, col2 = st.columns(2)
...
with col1:
    pie(df, "LABEL_13", "Há esgoto sanitário")
    pie(df, "LABEL_Q14", "Forma de coleta")
    pie(df, "LABEL_Q15", "Há pontos de vazamento")

with col2:
    pie(df, "LABEL_Q16", "Há mal cheiro")
    pie(df, "LABEL_Q17", "Rede de esgoto se mistura com rede pluvial")
    pie(df, "LABEL_Q18", "Lançamento de esgoto em corpos hídricos")
...
st.write("""
- Ações educativas  
- Instalação de rede de coleta de esgoto sanitário 
- Fossas sépticas coletivas
- Subsidio para instalação de fossas sépticas individuais
- Desativação de fossas negras
- Fiscalização de obras em residências e prédios comerciais
- Identificação de esgotamento irregular
""")